...
| **Kernel config** | Implemented (fragment) | `kernel/.config` + `.config.notes` — a `merge_config.sh`-style fragment, every `CONFIG_*` justified in `.config.notes`. Reconciled: `scripts/build-kernel.sh` downloads and builds Linux 6.6 LTS, matching DL-009. |
| **luna-init (PID 1)** | **Implemented** | 20 `.c`/`.h` files, C17, statically linked. Every Stage 0–4 subsystem is real: signalfd+epoll event loop, zombie reaper, fstab.toml mounting, custom TOML parser (fuzz-tested), service loader, Kahn's-algorithm dependency graph + cycle detection, async supervisor (100ms timerfd-driven `supervisor_pump()`, not blocking), cgroup v2 assignment, `setgid`/`setgroups(0,NULL)`/`setuid` privilege drop (correct order), inotify-based live service-file reload, Unix-socket JSON control server, orderly shutdown/reboot, panic→emergency-shell fallback. Direct /etc/passwd parsing eliminates static NSS risks. |
...
| **LunaGUI** | **Implemented** | Implemented in Rust 2021 (`src/lunagui-rs/`). Lightweight native GUI toolkit with widget tree, layout flows (VBox/HBox), button/canvas/label/scroll/textfield widgets over LGP wire protocol. |
| **luna-shell / luna-island** | **Implemented** | Implemented in Rust 2021 (`src/luna-shell-rs/`, `src/luna-island-rs/`). Provides desktop dock, taskbar, launcher, notifications, and keybindings. |
| **luna-ai-d (LUNA AI daemon)** | **Implemented** | Implemented in C17 (`src/luna-ai-d/`). Local AI daemon acting as an HTTP streaming client/proxy to Ollama with presence reporting. |
| **lpkg (package manager)** | **Implemented** | Implemented in C17 (`src/lpkg/`). Binary package manager with Ed25519 signature verification via libsodium, database index tracking, and fail-closed security. |
...
Limine  (boot/limine.conf — loads vmlinuz-mahina + initramfs-mahina.img)
  │
  ▼
Linux Kernel (6.6 LTS per build script and DL-009)
  │
  ▼
Initramfs
  │
